...
class Design extends Backend
{
	...
	public function resize_modifier($filename, $type, $width=0, $height=0, $set_watermark=false, $grayscale=false)
	{
		$resized_filename = $this->image->add_resize_params($filename, $type, $width, $height, $set_watermark, $grayscale);
		...
		elseif($type == 'category'){
			//return $this->config->root_url.'/'.$this->config->resized_categories_images_dir.$resized_filename_encoded.'?'.$this->config->token($resized_filename);
			return $this->config->root_url.'/'.$this->config->resized_categories_images_dir.$resized_filename_encoded;
		}
		elseif($type == 'brand')
			return $this->config->root_url.'/'.$this->config->resized_brands_images_dir.$resized_filename_encoded.'?'.$this->config->token($resized_filename);
		elseif($type == 'blog')
			return $this->config->root_url.'/'.$this->config->resized_blog_images_dir.$resized_filename_encoded;
		elseif($type == 'pages')
			return $this->config->root_url.'/'.$this->config->resized_pages_images_dir.$resized_filename_encoded;
		elseif($type == 'loans')
			return $this->config->root_url.'/'.$this->config->resized_loans_images_dir.$resized_filename_encoded;
		elseif($type == 'gallery')
			return $this->config->root_url.'/'.$this->config->resized_galleries_images_dir.$resized_filename_encoded;
	}
