...
class Orders extends Backend
{
	...
	public function get_order($id)
	{
		...
		$query = $this->db->placehold("SELECT  o.id, o.type, o.delivery_id, o.delivery_price, o.separate_delivery,
	...
	function get_orders($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT o.id, o.type, o.delivery_id, o.delivery_price, o.separate_delivery,
									...
									WHERE 1
									$id_filter $type_filter $status_filter $user_filter $keyword_filter $label_filter $modified_since_filter GROUP BY o.id ORDER BY status, id DESC $sql_limit", "%Y-%m-%d");
		$this->db->query($query);
		$orders = array();
	...
	function count_orders($filter = array())
	{
		...
		$query = $this->db->placehold("SELECT COUNT(DISTINCT id) as count
									...
									LEFT JOIN __orders_labels AS ol ON o.id=ol.order_id 
									WHERE 1
									$status_filter $type_filter $user_filter $label_filter $keyword_filter");
		$this->db->query($query);
