...
class Request extends Backend
{
	...
	{
		if(empty($this->this_url))
		{
			$this->this_url = $this->get('url', 'string');
		}
	...
	* Пример:
	* 
	*	if($backend->request->method('post'))
	...
	* например, чтобы получить имя загруженного файла: $filename = $backend->request->files('myfile', 'name');
	*/
...
//		}
	...
	*/
    ...
    public function array_to_key($arr, $keyname)
    {
