	...
	public function fetch()
	{
		...
		if($this->request->method('post'))
		{
			switch($this->request->post('action'))
			{
				case 'create':
				{
					$filename = $dir.'backend_'.date("Y_m_d_G_i_s").'.zip';
					##Дамп базы	
					$this->db->dump($dir.'backend.sql');
					chmod($dir.'backend.sql', 0777);
					
					### Архивируем
					$zip = new PclZip($filename);
					$v_list = $zip->create(array('files', $dir.'backend.sql'), PCLZIP_OPT_REMOVE_PATH, $dir, PCLZIP_CB_PRE_ADD, "myCallBack");
					if ($v_list == 0)
					{
			    ...
			    case 'restore':
			    {
					...
					elseif (!$zip->extract(PCLZIP_OPT_PATH, $dir, PCLZIP_OPT_BY_NAME, 'backend.sql'))
					{
						trigger_error('Не могу разархивировать '.$zip->errorInfo(true));
					}
					elseif (!is_readable($dir.'backend.sql'))
					{
						trigger_error('Не могу прочитать файл /temp/backend.sql');
					}
					else
					{
						$this->db->restore($dir.'backend.sql');
						unlink($dir.'backend.sql');
