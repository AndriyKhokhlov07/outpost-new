   	    		...
   	    		if($product->id)
   	    		{
					...
					if($images = $this->request->post('images'))
					{
		  		    ...
		  		    if($images = $this->request->files('images'))
		  		    {
						...
						{
				 			if ($image_name = $this->image->upload_image($images['tmp_name'][$i], $images['name'][$i], 'product'))
				 			{
					 		...
					 		elseif($dropped_images = $this->request->files('dropped_images'))
					  		{
							 	...
							 	if ($key!==false && $image_name = $this->image->upload_image($dropped_images['tmp_name'][$key], $dropped_images['name'][$key], 'product'))
						  	   				$this->products->add_image($product->id, $image_name);
							}
						}
					}
