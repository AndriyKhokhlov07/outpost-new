...
{else}
{$meta_title = 'New brand' scope=parent}
{/if}
...
<div class="message message_success">
	<span class="text">{if $message_success=='added'}{$brand->name|escape} was created{elseif $message_success=='updated'}{$brand->name|escape} was updated{else}{$message_success}{/if}</span>
	<a class="link" target="_blank" href="../brands/{$brand->url}">View brand page</a>
...
<div class="message message_error">
	<span class="text">{if $message_error=='url_exists'}Brand with the same URL already exists{else}{$message_error}{/if}</span>
	<a class="button" href="{$smarty.get.return}">Back</a>
</div>
<!-- Системное сообщение (The End)-->
{/if}
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>URL</label><div class="page_url"> /brands/</div><input name="url" class="page_url" type="text" value="{$brand->url|escape}" /></li>
				<li><label class=property>Title</label><input name="meta_title" class="simpla_inp" type="text" value="{$brand->meta_title|escape}" /></li>
				<li><label class=property>Keywords</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$brand->meta_keywords|escape}" /></li>
				<li><label class=property>Brand description</label><textarea name="meta_description" class="simpla_inp" />{$brand->meta_description|escape}</textarea></li>
			</ul>
		</div>
	...
	<!-- Brand description бренда (The End)-->
	<input class="button_green button_save" type="submit" name="" value="Save" />
