...
{capture name=tabs}
	...
	<li class="active"><a href="index.php?module=CategoriesAdmin">Categories</a></li>
...
<div class="message message_success">
	<span class="text">{if $message_success=='added'}{$category->name|escape} was created{elseif $message_success=='updated'}{$category->name|escape} was updated{else}{$message_success}{/if}</span>
	<a class="link" target="_blank" href="../catalog/{$category->url}">View category page</a>
...
<div class="message message_error">
	<span class="text">{if $message_error=='url_exists'}Category with the same URL already exists{else}{$message_error}{/if}</span>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
	<div id="name">
		...
		<div class="checkbox">
			<input name=visible value='1' type="checkbox" id="active_checkbox" {if $category->visible}checked{/if}/> <label for="active_checkbox">Enabled</label>
		</div>
	</div> 
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>URL</label><div class="page_url">/catalog/</div><input name="url" class="page_url" type="text" value="{$category->url|escape}" /></li>
				<li><label class=property>Title</label><input name="meta_title" class="simpla_inp" type="text" value="{$category->meta_title|escape}" /></li>
				<li><label class=property>Keywords</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$category->meta_keywords|escape}" /></li>
				<li><label class=property>Category description</label><textarea name="meta_description" class="simpla_inp">{$category->meta_description|escape}</textarea></li>
			</ul>
		</div>
