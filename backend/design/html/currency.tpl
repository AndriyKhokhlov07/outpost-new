{capture name=tabs}
	...
	<li class="active"><a href="index.php?module=CurrencyAdmin">Currencies</a></li>
...
{$meta_title = 'Currencies' scope=parent}
...
$(function() {
	...
	$("form").submit(function() {
		if($('input[type="hidden"][name="action"]').val()=='delete' && !confirm('Подтвердите удаление'))
			return false;	
		if(base_currency_id != $('input[name*="currency[id]"]:first').val() && confirm('Convert all prices to '+$('input[name*="name"]:first').val()+' using current rate?', 'msgBox Title'))
...
{/literal}
	...
	<div id="header">
		<h1>Currencies</h1>
		<a class="add" id="add_currency" href="#">Add currency</a>
	<!-- Заголовок (The End) -->	
	</div>	
	...
	<div id="currencies_block">
		<ul id="header">
			<li class="move"></li>
			<li class="name">Currency name</li>	
			<li class="icons"></li>	
			<li class="sign">Sign</li>	
			<li class="iso">ISO Code</li>	
		</ul>
		<div id="currencies">
		...
		<ul class="sortable {if !$c->enabled}invisible{/if} {if $c->cents == 2}cents{/if}">
			...
			<li class="icons">
				<a class="cents" href="#" title="Use cents on site"></a>
				<a class="enable" href="#" title="Show on site"></a>
			</li>
