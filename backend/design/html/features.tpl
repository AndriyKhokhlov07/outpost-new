...
{* Title *}
{$meta_title='Features' scope=parent}

{* Заголовок *}
<div id="header">
	<h1>Features</h1> 
	<a class="add" href="{url module=FeatureAdmin return=$smarty.server.REQUEST_URI}">Add new feature</a>
</div>	
...
<div id="main_list" class="features">
	...
	<input type="hidden" name="session_id" value="{$smarty.session.id}">
		...
		<div id="list">
			...
			<div class="{if $feature->in_filter}in_filter{/if} row">
				...
				<div class="icons cell">
					<a title="Use for products filter" class="in_filter" href='#' ></a>
					<a title="Delete" class="delete" href='#' ></a>
				</div>
				<div class="clear"></div>
			</div>
			{/foreach}
		...
		<span id="select">
		<select name="action">
			<option value="set_in_filter">Use for products filter</option>
			<option value="unset_in_filter">Do not use for products filter</option>
