{capture name=tabs}
	<li class="active"><a href="{url module=ProductsAdmin category_id=$product->category_id return=null brand_id=null id=null}">Products</a></li>
...
{else}
{$meta_title = 'New product' scope=parent}
{/if}
...
<div class="message message_success">
	<span class="text">{if $message_success=='added'}{$product->name|escape} was created{elseif $message_success=='updated'}{$product->name|escape} was updated{else}{$message_success|escape}{/if}</span>

	<a class="link" target="_blank" href="../products/{$product->url}">View product page</a>
	...
	{*
	<span class="share">		
		<a href="#" onClick='window.open("http://vkontakte.ru/share.php?url={$config->root_url|urlencode}/products/{$product->url|urlencode}&title={$product->name|urlencode}&description={$product->annotation|urlencode}&image={$product_images.0->filename|resize:'product':1000:1000|urlencode}&noparse=true","displayWindow","width=700,height=400,left=250,top=170,status=no,toolbar=no,menubar=no");return false;'>
...
<div class="message message_error">
	<span class="text">{if $message_error=='url_exists'}Product with the same URL already exists{elseif $message_error=='empty_name'}Name can't be blank{else}{$message_error|escape}{/if}</span>
	{if $smarty.get.return}
	<a class="button" href="{$smarty.get.return}">Back</a>
	{/if}
</div>
...
<input type=hidden name="session_id" value="{$smarty.session.id}">
 	...
 	<div id="name">
		...
		<div class="checkbox">
			<input name=visible value='1' type="checkbox" id="active_checkbox" {if $product->visible}checked{/if}/> <label for="active_checkbox">Visible</label>
		</div>
		<div class="checkbox">
			<input name=featured value="1" type="checkbox" id="featured_checkbox" {if $product->featured}checked{/if}/> <label for="featured_checkbox">Featured</label>
		</div>
	</div> 
	...
	<div id="product_brand" {if !$brands}style='display:none;'{/if}>
		...
		<select name="brand_id">
            <option value='0' {if !$product->brand_id}selected{/if} brand_name=''>No brand</option>
		...
		<div>
			<ul>
				...
				<li>
					...
					<span {if $smarty.foreach.categories.first}style='display:none;'{/if} class="delete"><i class="dash_link">Remove</i></span>
				</li>
				{/foreach}		
			</ul>
		</div>
	...
	<div id="variants_block" {assign var=first_variant value=$product_variants|@first}{if $product_variants|@count <= 1 && !$first_variant->name}class=single_variant{/if}>
		<ul id="header">
			...
			<li class="variant_name">Variant name</li>	
			<li class="variant_sku">SKU</li>	
			<li class="variant_price">Price, {$currency->sign}</li>	
			<li class="variant_discount">Compare, {$currency->sign}</li>	
			<li class="variant_amount">Qty</li>
		</ul>
		<div id="variants">
		...
		<ul>
			...
			<li class="variant_download">
				...
				{if $variant->attachment}
					...
					<a href='#' class=remove_attachment><img src='design/images/bullet_delete.png'  title="Remove цифровой товар"></a>
					<a href='#' class=add_attachment style='display:none;'><img src="design/images/cd_add.png" title="Add digital product" /></a>
				{else}
					<a href='#' class=add_attachment><img src="design/images/cd_add.png"  title="Add digital product" /></a>
				{/if}
		...
		<span class="add" id="add_variant"><i class="dash_link">Add product variant</i></span>
 	</div>
	...
	<div id="column_left">
		...
		<div class="block layer">
			...
			<ul>
				<li><label class=property>URL</label><div class="page_url"> /products/</div><input name="url" class="page_url" type="text" value="{$product->url|escape}" /></li>
				<li><label class=property>Title</label><input name="meta_title" class="simpla_inp" type="text" value="{$product->meta_title|escape}" /></li>
				<li><label class=property>Keywords</label><input name="meta_keywords" class="simpla_inp" type="text" value="{$product->meta_keywords|escape}" /></li>
				<li><label class=property>Description</label><textarea name="meta_description" class="simpla_inp" />{$product->meta_description|escape}</textarea></li>
			</ul>
		</div>
		<!-- Page properties (The End)-->
				
		<div class="block layer" {if !$categories}style='display:none;'{/if}>
			<h2>Product features
			<a href="#" id=properties_wizard><img src="design/images/wand.png" alt="Autofill" title="Autofill"/></a>
			</h2>
			...
			<span class="add"><i class="dash_link" id="add_new_feature">Add new feature</i></span>
			<input class="button_green button_save" type="submit" name="" value="Save" />			
		</div>
		
		<!-- Product features (The End)-->
	...
	<div id="column_right">
		...
		<div class="block layer images">
			<h2>Product images
			<a href="#" id=images_wizard><img src="design/images/wand.png" alt="Autofill" title="Autofill"/></a>
			</h2>
			<ul>{foreach $product_images as $image}<li>
					<a href='#' class="delete"><img src='design/images/cross-circle-frame.png'></a>
					<img src="{$image->filename|resize:'product':100:100}" alt="" />
					<input type=hidden name='images[]' value='{$image->id}'>
				</li>{/foreach}</ul>
			<div id=dropZone>
				<div id=dropMessage>Drop files to upload</div>
				<input type="file" name="dropped_images[]" multiple class="dropInput">
			</div>
			<div id="add_image"></div>
			<span class=upload_image><i class="dash_link" id="upload_image">Upload image</i></span> or <span class=add_image_url><i class="dash_link" id="add_image_url">download from url</i></span>
		</div>
		...
		<div class="block layer">
			...
			<input type=text name=related id='related_products' class="input_autocomplete" placeholder='Find product'>
		</div>
