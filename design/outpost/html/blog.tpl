	...
	<div class="blog_ch2">
	    <ul class="blog fx ch2">
	        ...
	        <li>
	        	...
	        	<div class="preview">
	                <h4><a data-post="{$post->id}" href="{if $post->url2}{$post->url2}{else}blog{if $post->url}/{/if}{$post->url}{/if}">{$post->name|escape}</a></h4>
	            ...
	            {if $post->image}
	                    <a class="img" href="{if $post->url2}{$post->url2}{else}blog{if $post->url}/{/if}{$post->url}{/if}">
	                        <img src="{$post->image|resize:blog:700:700}" title="Outpost Coliving Blog (Image-2), outpost-club.com" alt="Outpost Coliving Blog  (Image-2)" ">
	                    </a>
	            {/if}
	            <div class="preview">
	            	{if $post->tags_ids}
	                ...
	                <a class="more" href="{if $post->url2}{$post->url2}{else}blog{if $post->url}/{/if}{$post->url}{/if}" data-text="Read More"></a>
	            </div>
	        </li>
	        {/foreach}
	    </ul>
	...
	{include file='bx/house_list.tpl'} 
		...
		<ul class="featured_blog">
			...
			<li class="fx">
				...
				<div>
					<h5 class="h5"><a data-post="{$post->id}" href="{if $post->url2}{$post->url2}{else}blog{if $post->url}/{/if}{$post->url}{/if}">{$post->name|escape}</a></h5>
	                <p class="date">{$post->date|date_format:"%b %e, %Y"}</p>
				</div>
